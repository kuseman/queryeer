...
class BaseDatabase implements JdbcDatabase
{
    ...
    BaseDatabase(Icons icons, CatalogCrawlService crawlService, IEventBus eventBus, QueryActionsConfigurable queryActionsConfigurable)
    {
        this.treeNodeSupplier = new JdbcTreeNodeSupplier(this, icons, eventBus, queryActionsConfigurable);
    ...
    {
        return new PrestoDocumentParser(this, eventBus, queryActionsConfigurable, crawlService, connectionState);
    }
    ...
    public Catalog getCatalog(IConnectionState connectionState, String database)
    {
        ...
        try (Connection connection = connectionState.createConnection())
        {
            ...
            LOGGER.info("Fetched metadata for " + database + " in: " + DurationFormatUtils.formatDurationHMS(TimeUnit.MILLISECONDS.convert(System.nanoTime() - time, TimeUnit.NANOSECONDS)));
        }
