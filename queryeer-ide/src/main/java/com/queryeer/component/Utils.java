...
import com.queryeer.component.PropertyFields.PropertyField;
    ...
    static PropertyFields collectPropertyFields(Class<?> clazz)
    {
        try
        {
            ...
            for (PropertyDescriptor pd : beanInfo.getPropertyDescriptors())
            {
                ...
                if (property == null)
                {
                    // Switch to the class read method and see if there is a annotation there instead
                    if (readMethod.getDeclaringClass()
                            .isInterface())
                    {
                        try
                        {
                            readMethod = clazz.getDeclaredMethod(readMethod.getName(), readMethod.getParameterTypes());
                ...
                result.add(new PropertyField(pd, property, readMethod, null));
            }
