...
/** Test of {@link PLBDocumentParser} */
public class PLBDocumentParserTest
{
    // @Test
    // public void test_regression_in_c3_where_suggests_would_hang_becuase_of_large_parse_tree() throws Exception
    // {
    // String query = IOUtils.toString(PLBParserTest.class.getResourceAsStream("/com/queryeer/payloadbuilder/query_with_many_and_clauses.plbsql"), StandardCharsets.UTF_8);
    //
    ...
    //
    // // After "and " line 28
    // suggestions = parser.getSuggestions(new StringReader(query), 641);
    ...
    // // suggestions = parser.getSuggestions(new StringReader(query), 1110);
    // // assertEquals("log.logger", suggestions.textToMatch());
    // // assertEquals(false, suggestions.skipRules());
    // // assertEquals(Set.of(PayloadBuilderQueryParser.RULE_primary), suggestions.collection().rules.keySet());
    //
    // // After "._doc" line 25
    // suggestions = parser.getSuggestions(new StringReader(query), 621);
    ...
    // {
    // PLBDocumentParser parser = new PLBDocumentParser(new QuerySession(new CatalogRegistry()));
    // return parser.getSuggestions(new StringReader(query), offset);
    // }
}
