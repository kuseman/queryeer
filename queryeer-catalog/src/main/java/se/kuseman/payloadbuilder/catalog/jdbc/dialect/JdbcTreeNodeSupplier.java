...
import se.kuseman.payloadbuilder.catalog.TemplateUtils;
...
class JdbcTreeNodeSupplier implements TreeNodeSupplier
{
    ...
    JdbcTreeNodeSupplier(JdbcDatabase jdbcDatabase, Icons icons, IEventBus eventBus, QueryActionsConfigurable queryActionsConfigurable)
    {
        this.icons = icons;
    ...
    private void getQueryActions(JdbcConnection jdbcConnection, String database, List<Action> tableContextMenuActions, List<Action> tableLinkActions, ObjectType objectType, Map<String, Object> model)
    {
        List<QueryAction> queryActions = queryActionsConfigurable.getQueryActions(jdbcDatabase, ActionTarget.NAVIGATION_TREE);
        for (QueryActionsConfigurable.QueryAction queryAction : queryActions)
        {
            if (!queryAction.getObjectTypes()
                    .contains(objectType))
            {
                continue;
            }
            ...
            if (queryAction.getType() == ActionType.LINK)
            {
                tableLinkActions.add(action);
            }
            else if (queryAction.getType() == ActionType.CONTEXT_MENU)
            {
                tableContextMenuActions.add(action);
            }
        }
    }

    private Action buildAction(JdbcConnection jdbcConnection, String database, QueryActionsConfigurable.QueryAction queryAction, Map<String, Object> model)
    {
        Action action;
                ...
                .isEmpty())
        {
            ...
            action = new AbstractAction(queryAction.getTitle())
            {
                ...
                {
                    eventBus.publish(
                            new ExecuteQueryEvent(queryAction.getOutput(), new ExecuteQueryContext(jdbcConnection, jdbcDatabase, database, TemplateUtils.process(queryAction.title, query, model))));
                }
            };
        }
        else
    ...
    protected RegularNode buildTableNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.TABLE,
                Map.of("catalog", Objects.toString(tableName.catalog, ""), "schema", Objects.toString(tableName.schema, ""), "name", Objects.toString(tableName.name, "")));

        modifyTableContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifyTableLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        // String tableNodeName = getTableName(rs);
        return new QueryeerTree.TreeNode(tableName.title, icons.tableIcon, () -> buildTableChildNodes(jdbcConnection, sqlConnectionSupplier, database, tableName), contextMenuActions, linkActions);
    }
    ...
    protected void modifyTableLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }

    protected void modifyTableContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
    ...
    protected RegularNode buildViewNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        List<Action> contextMenuActions = new ArrayList<>();
        List<Action> linkActions = new ArrayList<>();
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.VIEW,
                Map.of("catalog", Objects.toString(viewName.catalog, ""), "schema", Objects.toString(viewName.schema, ""), "name", Objects.toString(viewName.name, "")));

        modifyTableContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifyTableLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        return new QueryeerTree.TreeNode(viewName.title, icons.viewIcon, () -> buildViewChildNodes(jdbcConnection, sqlConnectionSupplier, database, viewName), contextMenuActions, linkActions);
    }

    protected void modifyViewLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }

    protected void modifyViewContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
    ...
    protected RegularNode buildSynonymNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        List<Action> contextMenuActions = new ArrayList<>();
        List<Action> linkActions = new ArrayList<>();
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.SYNONYM,
                Map.of("catalog", Objects.toString(synonymName.catalog, ""), "schema", Objects.toString(synonymName.schema, ""), "name", Objects.toString(synonymName.name, "")));

        modifySynonymContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifySynonymLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        return new QueryeerTree.TreeNode(synonymName.title, icons.viewIcon, () -> buildSynonymChildNodes(jdbcConnection, sqlConnectionSupplier, database, synonymName), contextMenuActions,
                linkActions);
    }

    protected void modifySynonymLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }

    protected void modifySynonymContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
    ...
    protected RegularNode buildTriggerNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        List<Action> contextMenuActions = new ArrayList<>();
        List<Action> linkActions = new ArrayList<>();
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.TRIGGER, Map.of("name", triggerName));

        modifyTableContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifyTableLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        return new QueryeerTree.TreeNode(triggerName, icons.boltIcon, () -> buildTriggerChildNodes(jdbcConnection, sqlConnectionSupplier, database, triggerName), contextMenuActions, linkActions);
    }
    ...
    protected void modifyTriggerLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }

    protected void modifyTriggerContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
    ...
    protected RegularNode buildProcedureNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        List<Action> contextMenuActions = new ArrayList<>();
        List<Action> linkActions = new ArrayList<>();
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.PROCEDURE,
                Map.of("catalog", Objects.toString(procedureName.catalog, ""), "schema", Objects.toString(procedureName.schema, ""), "name", Objects.toString(procedureName.name, "")));

        modifyProcedureContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifyProcedureLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        return new QueryeerTree.TreeNode(procedureName.title, icons.wpformsIcon, () -> buildProcedureChildNodes(jdbcConnection, sqlConnectionSupplier, database, procedureName), contextMenuActions,
                linkActions);
    }
    ...
    protected void modifyProcedureContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions)
            throws SQLException
    {
    }

    protected void modifyProcedureLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
    ...
    protected RegularNode buildFunctionNode(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database) throws SQLException
    {
        ...
        List<Action> contextMenuActions = new ArrayList<>();
        List<Action> linkActions = new ArrayList<>();
        getQueryActions(jdbcConnection, database, contextMenuActions, linkActions, ObjectType.FUNCTION,
                Map.of("catalog", Objects.toString(functionName.catalog, ""), "schema", Objects.toString(functionName.schema, ""), "name", Objects.toString(functionName.name, "")));

        modifyProcedureContextMenuActions(jdbcConnection, sqlConnectionSupplier, rs, database, contextMenuActions);
        modifyProcedureLinkActions(jdbcConnection, sqlConnectionSupplier, rs, database, linkActions);

        return new QueryeerTree.TreeNode(functionName.title, icons.fileTextIcon, () -> buildFunctionChildNodes(jdbcConnection, sqlConnectionSupplier, database, functionName), contextMenuActions,
                linkActions);
    }
    ...
    protected void modifyFunctionContextMenuActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }

    protected void modifyFunctionLinkActions(JdbcConnection jdbcConnection, SqlConnectionSupplier sqlConnectionSupplier, ResultSet rs, String database, List<Action> actions) throws SQLException
    {
    }
