...
/** Add search button for {@link TextOutputComponent} */
class FindTextToolbarActionFactory implements IOutputToolbarActionFactory
{
    @Override
    public List<IExtensionAction> create(final IOutputComponent outputcomponent)
    {
        if (!(outputcomponent instanceof TextOutputComponent))
        {
            return emptyList();
        }
            ...
            public Action getAction()
            {
                if (action == null)
                {
                    action = new AbstractAction()
                    {
                        ...
                        {
                            ((TextOutputComponent) outputcomponent).showFind();
                        }
                    };
                }
                return action;
