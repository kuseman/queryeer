...
class CatalogExtensionView extends JPanel
{
    ...
    CatalogExtensionView(IEventBus eventBus, IQueryFileProvider queryFileProvider, ICatalogExtension extension, String alias, ButtonGroup defaultGroup)
    {
        ...
        rbDefault.addActionListener(l ->
        {
                    ...
                    && fireEvents)
            {
                ...
                if (file != null)
                {
                    Optional<IPayloadbuilderQueryFileState> state = file.getQueryFileState(IPayloadbuilderQueryFileState.class);
                    if (state.isPresent())
                    {
                        ((QuerySession) state.get()
                                .getSession()).setDefaultCatalogAlias(labelAlias.getText());
                    }
                }
            }
        });
    ...
    {
        IQueryFile queryFile = event.getQueryFile();
        // Update default catalog radio if the current query file finished executing
